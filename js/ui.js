...
export function openData() {
  openSheet('Data', `
    <p class="small muted">Everything is stored on this device only. Nothing is uploaded. Export regularly — clearing browser data for this site erases the file.</p>
    <div class="spacer"></div>
    <button class="ghost" id="do-export" style="width:100%">Export JSON</button>
    <div class="spacer"></div>
    <label class="field">
      <span class="lab">Import a previous export (merges by default)</span>
      <input type="file" id="do-import" accept="application/json,.json">
    </label>
    <label class="field" style="display:flex;gap:10px;align-items:center">
      <input type="checkbox" id="import-replace" style="width:auto">
      <span class="small">Replace everything instead of merging</span>
    </label>
    <div class="spacer"></div>
    <button class="ghost" id="do-wipe" style="width:100%;color:var(--bad)">Erase all data</button>
    <div class="spacer"></div>
    <p class="tiny muted">${M.state.observations.length} observations · ${M.state.entities.length} entities · ${M.state.questions.length} questions</p>`);

  document.getElementById('do-export').addEventListener('click', () => {
    const blob = new Blob([JSON.stringify(M.exportPayload(), null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `spy-work-${today()}.json`;
    a.click();
  ...
  document.getElementById('do-import').addEventListener('change', async e => {
    ...
    if (!file) return;
    try {
      const result = await M.importPayload(JSON.parse(await file.text()), { replace: document.getElementById('import-replace').checked });
      closeSheet();
      render();
      toast(`Imported ${result.observations} observations.`);
    } catch (err) {
  ...
  document.getElementById('do-wipe').addEventListener('click', async () => {
    if (!confirm('Erase every observation, entity and question on this device?')) return;
    await M.wipe();
    closeSheet();
    render();
    toast('Erased.');
